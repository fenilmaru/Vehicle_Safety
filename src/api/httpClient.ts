 ...
 * 
 * **Django backend** — all /api/* calls are proxied by Next.js to Django at :8000.
 * JWT tokens are stored in localStorage and sent as `Authorization: Bearer ...`.
 */
...
export const http = {
  ...
  async request<T>(path: string, config: RequestConfig = {}): Promise<T> {
    ...
    // Proxy to Django backend via Next.js rewrite
    const url = `/api${path}${query}`;
    requestInterceptors.forEach((fn) => fn(url, config));

    // Add Django JWT Bearer token
